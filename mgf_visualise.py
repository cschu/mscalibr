...
def binSpectraByPrecursorMass(fi, winsize=200):
    bins = {}
    with mgf.read(fi) as reader:
        for spectrum in reader:
            pmass = spectrum['params']['pepmass'][0]
            bin_ = int(pmass // winsize)
            if bin_ not in bins:
...
def binSpectraByCharge(fi):
    bins = {}
    with mgf.read(fi) as reader:
        for spectrum in reader:
            pmass = spectrum['params']['pepmass'][0]
            bin_ = spectrum['params']['charge'][0]
            if bin_ not in bins:
                bins[bin_] = [[], [], []]
            bins[bin_][0].extend(spectrum['intensity array'])
            # bins[bin_][1].extend(spectrum['m/z array'])
...
def makePlots(fi1, fi2, figfn, winsize=200):
    bins1, bins2 = map(lambda x:binSpectraByPrecursorMass(x), [fi1, fi2])
    for bin_ in sorted(set(bins1).intersection(set(bins2))):
        ...
        plot1.set_title('%s\n%i - %i D' % ((os.path.basename(fi1),) + interval))
        plot2.set_title('%s\n%i - %i D' % ((os.path.basename(fi2),) + interval))
        ...
        fig.tight_layout()
        fig.savefig('%s_%i.png' % (figfn, interval[1]), dpi=300)
    pass


#bins1 = binSpectraByPrecursorMass(sys.argv[1])
#bins2 = binSpectraByPrecursorMass(sys.argv[2])
makePlots(sys.argv[1], sys.argv[2], 'pmass')
...
#def filterSpectrum(spectrum, f):
#    return spectrum if f(spectrum) else None
